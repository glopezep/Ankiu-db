...
class Db {
  ...
  static async getUser(id, callback) {
    try {
      const result = await models.User.forge({ id }).fetch({ withRelated: ['role'] });
      return Promise.resolve(result).asCallback(callback);
    } catch (e) {
      return Promise.reject(e).asCallback(callback);
    }
  ...
  static async getUsers(callback) {
    try {
      const users = await collections.Users.forge().fetch();
  ...
  static async getProduct(id, callback) {
    try {
      const product = await models.Product.forge({ id }).fetch({ withRelated: ['category'] });
